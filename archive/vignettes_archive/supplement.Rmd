...
a <- ggplot() +
  ...
  labs(x = 'Longitude', y = 'Latitude') +
  theme_void()
get_correlation(prism, prism_patterns)[states_wus]
b <- ggplot() +
  geom_stars(data = st_crop(get_correlation(cera, cera_patterns), states_wus) %>% 
               split() %>% 
               mutate(PC1 = PC1 * -1, PC2 = PC2 * -1, PC4 = PC4 * -1) %>% 
               merge(name= 'PC')) + # the signs are arbitrary, so adjust the cera signs so the colors match prism)
  geom_sf(data = states_wus, fill = NA, color = 'black') +
  facet_wrap(~PC, nrow = 1) +
  scale_fill_scico(palette = 'broc', limits = c(-1, 1), name = 'Correlation', na.value = NA) +
  labs(x = 'Longitude', y = 'Latitude') +
...
c <- ggplot() +
  geom_stars(data = st_crop(get_correlation(ccsm, ccsm_patterns), states_wus) %>% 
               split() %>%
               mutate(PC1 = PC1 * -1,
                      PC2 = PC2 * -1,
                      PC3 = PC3 * -1) %>% 
...
prism_mon <- system.file('testdata/prism_test.RDS', package = 'tidyEOF') %>%
  readRDS()

get_patterns(filter(prism_mon), k = 1) %>% reconstruct_field(nonneg = FALSE) %>% `-`(filter(prism_mon)) %>%st_apply(1:2, function(x) sqrt(mean(x^2))) %>% plot

get_patterns(filter(prism_mon), k = 36)$amplitudes

profvis({
  get_climatology(prism)
})
 
expand_grid(kx = 5, ky = 5, kxy = 5) %>%
  filter(kxy <= pmin(kx, ky)) %>%
  mutate(cv = pmap(list(kx, ky, kxy), ~prep_cca(..1, ..2, 
                                                preds = cera, 
                                                obs = prism) %>% 
...
units::set_units(prism, 'mm')
prism %>%
      purrr::modify2(prism, ~ units::set_units(.x, units(.y), mode = 'standard')) %>%
      setNames(names(prism)) %>%
      st_as_stars(dimensions = st_dimensions(prism))

prism %>%
      purrr::map2_dfc(prism, ~ units::set_units(.x, units(.y), mode = 'standard')) %>%
      setNames(names(prism)) %>%
      st_as_stars(dimensions = st_dimensions(prism))
...
recon_20c_time <- predict_cca(get_patterns(filter(cera, time >= 1982), 5), 
            ...
            filter(cera), # stopgap for nonzero dimension start
            k = 5)
