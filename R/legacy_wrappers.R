...
fit_model <- function(data_in) {
  ...
  # Return the legacy implementation for backward compatibility
  # but issue deprecation warning
  fit_model_legacy(data_in)
}
...
fit_pcr <- function(data_in) {
  ...
  # Return the legacy implementation for backward compatibility
  # but issue deprecation warning
  fit_pcr_legacy(data_in)
}

#' Convert New Coupled Patterns to Legacy Format
#'
#' Utility function to convert new coupled_patterns objects to something
#' resembling the old format for compatibility.
#'
...
as_legacy_coupled_patterns <- function(coupled_patterns) {
  ...
  # Create a structure similar to the old GAM-based format
  # This is a best-effort conversion and may not capture all original functionality

  # Get canonical correlations as a proxy for R-squared values
  ...
  # Create legacy-style output
  legacy_format <- list(
    model = model_data,
    data = NULL,  # Not easily convertible without rerunning the analysis
...
#'
#' This function provides guidance on how to migrate from legacy functions
#' to the new CCA-based architecture.
#'
