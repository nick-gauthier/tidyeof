...
`tune_eof()` evaluates reconstruction skill across a range of `k` values. For each fold, it extracts patterns from training data, projects test data onto those patterns, reconstructs the test field, and compares against the held-out observations.
